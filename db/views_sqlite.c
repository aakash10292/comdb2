...
int hash_views_create_triggers(hash_view_t *view, sqlite3 *db, struct errstat *err)
{
    typedef char *(*PFUNC)(hash_view_t *, struct errstat *);

    PFUNC funcs[3] = {(PFUNC)&hash_views_create_delete_trigger_query, (PFUNC)&hash_views_create_update_trigger_query,
                      (PFUNC)&hash_views_create_insert_trigger_query};
    PFUNC func;
    char *stmt_str;
    int i;
    int rc;
...
int hash_views_sqlite_add_view(hash_view_t *view, sqlite3 *db, struct errstat *err)
{
    ...
    rc = hash_views_create_triggers(view, db, err);
