...
int replicant_can_retry(struct sqlclntstate *clnt)
{
    return clnt->dbtran.mode != TRANLEVEL_SNAPISOL &&
           ...
           !clnt->has_recording && !clnt->dbtran.trans_has_sp;
}
...
static inline int check_user_password(struct sqlclntstate *clnt)
{
    ...
    password_rc = bdb_user_password_check(tran, clnt->current_user.name,
                                          clnt->current_user.password,
                                          &valid_user);
        ...
        if (rc == SQLITE_OK && gbl_old_column_names && rec->stmt &&
            ...
            if (!t || do_reprepare) {
                ...
                if (rec->stmt && sqlite3_column_count(rec->stmt) == column_count)
                    stmt_set_cached_columns(rec->stmt, column_names,
                                            column_decltypes, column_count);
            }
        }
