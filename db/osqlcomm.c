...
static int _process_single_table_sc_mod_partitioning(struct ireq *iq) 
{
    ...
    sc->newshard = create_mod_view(sc->tablename, sc->partition.u.mod.column,sc->partition.u.mod.num_shards, sc->partition.u.mod.keys, sc->partition.u.mod.shards, &err);
    if (!sc->newshard) {
        logmsg(LOGMSG_ERROR,
