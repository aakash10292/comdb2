...
void comdb2DropTable(Parse *pParse, SrcList *pName, int dropPartitions)
{
...
void comdb2CreateHashPartition(Parse *pParse, IdList *pColumn, IdList *pPartitions, int createPartitions)
{
    struct comdb2_partition *partition;
    if (!gbl_partitioned_table_enabled) {
        setError(pParse, SQLITE_ABORT, "Create partitioned table not enabled");
    ...
    /* TODO : ENABLE THIS THROUGH SQL
     * We seem to have hit a limit in sqlite on the number of tokens
     * The result is - adding more tokens crashes the planner in random places.
     * In the interest on not being blocked for now, i've decided to proceed with a tunable
     * I will revisit and fix the broader issue later
     */
    if (createPartitions){
