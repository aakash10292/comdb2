...
partition_options ::= HASH LP idlist(A) RP PARTITIONS LP idlist(B) RP. {
    comdb2CreateHashPartition(pParse, A, B);
}
merge ::= .
merge ::= merge_with.
...
%ifdef SQLITE_BUILDING_FOR_COMDB2
  ...
  GRANT HASH INCLUDE INCREMENT IPU ISC KW LUA LZ4 MANUAL MERGE NONE
