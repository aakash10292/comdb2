...
partition_options ::= LP idlist(A) RP PARTITIONS LP idlist(B) RP create_partitions(C). {
    comdb2CreateHashPartition(pParse, A, B, C);
}
merge ::= .
merge ::= merge_with.
...
%ifdef SQLITE_BUILDING_FOR_COMDB2
  ...
  CREATEPARTITIONS DATA DATABLOB DATACOPY DBPAD DEFERRABLE DETERMINISTIC DISABLE 
  DISTRIBUTION DRYRUN DROPPARTITIONS ENABLE EXCLUSIVE_ANALYZE EXEC EXECUTE FORCE 
...
//
%ifdef SQLITE_BUILDING_FOR_COMDB2
ccons ::= AUTOINCR.              {comdb2AddAutoIncrement(pParse);}
...
cmd ::= drop_table.
%ifdef SQLITE_BUILDING_FOR_COMDB2
drop_table ::= dryrun DROP TABLE ifexists(E) fullname(X) drop_partitions(D). {
  sqlite3DropTable(pParse, X, 0, E, D);
}
%endif
%ifndef SQLITE_BUILDING_FOR_COMDB2
...
%ifdef SQLITE_BUILDING_FOR_COMDB2
  sqlite3DropTable(pParse, X, 1, E, 0);
%endif
%ifndef SQLITE_BUILDING_FOR_COMDB2
...
alter_table_alter_column_cmd ::= SET AUTOINCR. {
  comdb2AlterColumnSetAutoIncrement(pParse);
}
